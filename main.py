...
class Run_Experiments(ProcessDataset):
    def __init__(self, knn_params, balanced_data = False , apply_PCA = False, PCA_components = 15, directory = ''):
    ...
    def run(self, show_info = True):
        ...
        knn_experiment = KNNExperiment(X_norm, Y_readed, 5, self.text, self.directory)
...
def main():
    ...
    for param in PCA_parameters:
        experiments = Run_Experiments(knn_params, balanced_data=False, apply_PCA=param, PCA_components=15)
